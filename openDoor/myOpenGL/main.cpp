...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		vertices[10]=cos(temp*PI/180+PI)+1;
		//vertices[11]=2*sqrt(1-vertices[10]*vertices[10]);
		vertices[12]=sin(temp*PI/180+PI);
		vertices[15]=cos(temp*PI/180+PI)+1;
		vertices[17]=sin(temp*PI/180+PI);
		vertices[20]=cos(-temp*PI/180)-1;
		vertices[22]=sin(-temp*PI/180);
		vertices[25]=cos(-temp*PI/180)-1;
		vertices[27]=sin(-temp*PI/180);
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
		...
		model=glm::scale(model,glm::vec3(0.56,0.56,1.0));
		projection=glm::perspective(45.f,1.f,0.001f,100.0f);//45.0f, 1.0f, 0.1f, 100.0f
		ourShader.setMat4("projection",glm::value_ptr(projection));
		ourShader.setMat4("view",glm::value_ptr(view));
		ourShader.setMat4("model",value_ptr(model));
		glBindVertexArray(VAO);
