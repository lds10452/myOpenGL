...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		vertices[0]=sqrt(5.f)*cos(temp*PI/240-w);
		vertices[1]=sqrt(5.f)*sin(temp*PI/240-w)-1;
		vertices[5]=cos(temp*PI/240-PI/2);
		vertices[6]=sin(temp*PI/240-PI/2)-1;
		vertices[15]=2*cos(temp*PI/240);
		vertices[16]=2*sin(temp*PI/240)-1;

		vertices[20]=2*cos(-temp*PI/240+PI);
		vertices[21]=2*sin(-temp*PI/240+PI)-1;
		vertices[30]=cos(-temp*PI/240+3*PI/2);
		vertices[31]=sin(-temp*PI/240+3*PI/2)-1;
		vertices[35]=sqrt(5.f)*cos(-temp*PI/240+w+PI);
		vertices[36]=sqrt(5.f)*sin(-temp*PI/240+PI+w)-1;
		glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
